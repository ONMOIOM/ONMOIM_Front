...
const getStartTime = (d: any) =>
  d?.startTime ?? d?.schedule?.startDate ?? null;
const getEndTime = (d: any) =>
  d?.endTime ?? d?.schedule?.endDate ?? null;
...
export const EventPostLeftPanel = forwardRef<EventPostLeftPanelRef, Props>(
  ...
  const applyEdit = (field: string) => {
    ...
    setEditValue("");
    if (field === "title") setEventData({ ...eventData, title: trimmed || eventData.title });
    if (field === "introduction") setEventData({ ...eventData, introduction: trimmed || null, information: trimmed || eventData.information });
  };
    ...
    <div className="w-full mt-[192px] pl-[161px]">
      ...
      <div className="mt-[10px] text-h5 text-[#1A1A1A]">
        <span
          ...
          onKeyDown={(e) => isMyEvent && e.key === "Enter" && setOpenModal("schedule")}
          className={isMyEvent ? "cursor-pointer hover:bg-gray-100 rounded" : ""}
        >
      ...
      <div className="mt-[52px] space-y-[17px] text-[14px] font-medium text-[#1A1A1A]">
        <div className="flex items-start gap-[8px]">
          ...
          <span
            ...
            onKeyDown={(e) => isMyEvent && e.key === "Enter" && setOpenModal("location")}
            className={isMyEvent ? "cursor-pointer hover:bg-gray-100 rounded" : ""}
          >
            {locationText}
          </span>
        ...
        <div className="flex items-start gap-[8px]">
          ...
          <span
            ...
            onKeyDown={(e) => isMyEvent && e.key === "Enter" && setOpenModal("price")}
            className={isMyEvent ? "cursor-pointer hover:bg-gray-100 rounded" : ""}
          >
            {priceText}
          </span>
        ...
        <div className="flex items-start gap-[8px]">
          ...
          <span
            ...
            onKeyDown={(e) => isMyEvent && e.key === "Enter" && setOpenModal("seats")}
            className={isMyEvent ? "cursor-pointer hover:bg-gray-100 rounded" : ""}
          >
            {capacityText}
          </span>
        ...
        {(eventData.playlistUrl || eventData.playlist || isMyEvent) && (
          <div className="flex items-start gap-[8px]">
            <img src={playlist_icon} alt="playlist" className="w-[24px] h-[24px] shrink-0" />
            <span
              role="button"
              tabIndex={0}
              onClick={() => isMyEvent && setOpenModal("playlist")}
              onKeyDown={(e) => isMyEvent && e.key === "Enter" && setOpenModal("playlist")}
              className={isMyEvent ? "cursor-pointer hover:bg-gray-100 rounded" : ""}
            >
              {eventData.playlistUrl || eventData.playlist || "플레이리스트 없음"}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="mt-[48px] w-[645px]">
        {editingField === "introduction" && isMyEvent ? (
          ...
          <p
            role="button"
            tabIndex={0}
            onClick={() => isMyEvent && startEdit("introduction", eventData.introduction || eventData.information)}
            onKeyDown={(e) => isMyEvent && e.key === "Enter" && startEdit("introduction", eventData.introduction || eventData.information)}
          ...
          >
            {eventData.introduction || eventData.information || (isMyEvent ? "클릭하여 입력" : "")}
          </p>
        )}
      </div>
      ...
      <ScheduleModal
        ...
        value={eventData ? getScheduleValue() : { startAt: null, endAt: null }}
        onSave={(next) => {
          setEventData({
            ...eventData,
            startTime: next.startAt?.toISOString() ?? null,
            endTime: next.endAt?.toISOString() ?? null,
            schedule: next.startAt && next.endAt
              ? { startDate: next.startAt.toISOString(), endDate: next.endAt.toISOString() }
              : eventData.schedule,
          });
        }}
      />
      <LocationModal
        ...
        onClose={() => setOpenModal(null)}
        value={eventData ? getLocationValue() : { streetAddress: "", lotNumber: null }}
        onSave={(next) => {
          setEventData({
            ...eventData,
      ...
      <PlaylistModal
        ...
        onSave={(next) => setEventData({ ...eventData, playlistUrl: next, playlist: next })}
      />
    </div>
  );
});
